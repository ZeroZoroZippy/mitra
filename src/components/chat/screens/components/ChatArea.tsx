...
import { getMessages, saveMessage } from '../../../../../../shared/utils/firebaseDb';
// import { saveMessage, getMessages } from "../../../shared/utils/firebaseDb";
// import { auth } from "../../../shared/utils/firebaseAuth"; // ✅ Import Auth to get user ID
...
const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      if (user) {
        ...
        const formattedMessages: ChatMessage[] = loadedMessages.map((doc: any) => ({
          text: doc.text,
          sender: doc.sender,
  ...
  const handleSendMessage = async () => {
    ...
    if (user) {
      ...
      saveMessage(user.uid, userMessage.text, userMessage.sender).catch((error) =>
      ...
      {isWelcomeActive ? (
        <div className="welcome-container">
          ...
          <div className="suggestion-buttons">
            <button className="suggestion-button" onClick={() => handleWelcomeSuggestion("I had this interesting dream last night...")}>
              Talk about a dream
            </button>
            <button className="suggestion-button" onClick={() => handleWelcomeSuggestion("I could use some advice about...")}>
              I need advice
            </button>
            <button className="suggestion-button" onClick={() => handleWelcomeSuggestion("I've been feeling uncertain about...")}>
              Self-doubt
            </button>
            <button className="suggestion-button" onClick={() => handleWelcomeSuggestion("Hey, can we just chat?")}>
              Just talk...
            </button>
          </div>
        </div>
        ...
        <div className="messages-container">
          {messages.map((message, index) => (
            <div key={index} className={`message-bubble ${message.sender}-bubble`}>{message.text}</div>
          ))}
          {showTypingIndicator && (
            <div className="ai-typing-message">
              Mitra is typing<span className="ellipsis"><span>.</span><span>.</span><span>.</span></span>
            </div>
          )}
